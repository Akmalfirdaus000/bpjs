...
import { Search, Printer, Calendar, Clock, Play, CheckCircle, XCircle } from 'lucide-react';
...
export default function LaporanBkpsdm({ pensiunans, filters }: Props) {
    ...
    const handlePrintReport = (e: React.FormEvent) => {
        e.preventDefault();
        const url = `/pimpinan/laporan-bkpsdm/cetak?start_date=${startDate}&end_date=${endDate}&status=${reportStatus}`;
    ...
    return (
        <>
            <Head title="Laporan Pengajuan Pensiun (BKPSDM)" />
            <div className="flex h-full flex-1 flex-col gap-6 p-6">
                <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
                    <div>
                        <h1 className="text-2xl font-bold text-neutral-900 dark:text-emerald-700">
                            Laporan Pengajuan Pensiun (BKPSDM)
                        </h1>
                        <p className="text-sm text-neutral-500">
                            Monitoring seluruh usulan pemberhentian / pensiunan PNS Kota Padang.
                        </p>
                    </div>
                ...
                <Card className="border border-neutral-200 dark:border-neutral-800">
                    <CardHeader className="p-4 flex flex-col md:flex-row gap-4 items-center justify-between">
                        ...
                        <div className="flex items-center gap-2 w-full md:max-w-xs justify-end">
                            <Label htmlFor="filter-status" className="shrink-0 text-xs font-semibold text-neutral-500">STATUS:</Label>
                            ...
                            <table className="w-full text-sm text-left text-neutral-500 dark:text-neutral-400">
                                <thead className="text-xs text-neutral-700 uppercase bg-neutral-50 dark:bg-emerald-950/40 dark:text-emerald-700 border-b border-neutral-200 dark:border-emerald-900/40">
                                    <tr>
                                        ...
                                        <th scope="col" className="px-6 py-4 text-center">TMT Pensiun</th>
                                    ...
                                    {pensiunans.data.length === 0 ? (
                                        <tr>
                                            <td colSpan={7} className="px-6 py-8 text-center text-neutral-400">
                                                Tidak ada data pengajuan pensiun.
                                            </td>
                                        </tr>
                                    ) : (
                                        pensiunans.data.map((pensiunan, idx) => (
                                            <tr key={pensiunan.id} className="hover:bg-neutral-50/50 dark:hover:bg-emerald-950/20 transition-colors">
                                                ...
                                                <td className="px-6 py-4">{pensiunan.satuan_kerja}</td>
                                                <td className="px-6 py-4 font-mono">{formatRupiah(pensiunan.gaji_pokok)}</td>
                                                ...
                                                <td className="px-6 py-4 text-center">{renderStatusBadge(pensiunan.status)}</td>
                                            </tr>
                                        ))
                                    )}
                                </tbody>
            ...
            <Dialog open={isReportOpen} onOpenChange={setIsReportOpen}>
                <DialogContent className="max-w-md">
                    <DialogHeader>
                        <DialogTitle>Cetak Laporan Rekapitulasi BKPSDM</DialogTitle>
                    </DialogHeader>
                    <form onSubmit={handlePrintReport} className="space-y-4">
                        <div className="grid grid-cols-2 gap-4">
                            <div className="space-y-1">
                                <Label htmlFor="start_date">Tanggal Awal TMT</Label>
                                <Input
                                    id="start_date"
                                    type="date"
                                    value={startDate}
                            ...
                            <div className="space-y-1">
                                <Label htmlFor="end_date">Tanggal Akhir TMT</Label>
                                <Input
                                    id="end_date"
                                    type="date"
                                    value={endDate}
